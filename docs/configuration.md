...
Combining trusted proxy authentication with role-scoped plan visibility, using Microsoft Entra ID as the identity provider behind oauth2-proxy.
...
With this configuration, users in the viewer group sign in and see dashboards, environment status, check history and durations, with plan output restricted; users in the editor or admin groups additionally see raw plan output on environment pages.
